...
# region BatchNorm2d
class BatchNorm2d(ParamLayer, ABC):
    #  https://pytorch.org/docs/stable/generated/torch.nn.BatchNorm2d.html
    def __init__(self, num_features: int):
        super().__init__(weight_shape=(1, 1, 1, num_features),
                         weight_initializer=Constant(1.),
                         bias_initializer=Constant(0.)
                         )
    ...
    def forward(self, x, eval=False):
        if not isinstance(x, np.ndarray):
            x = np.array(x)
        assert len(x.shape) == 4, "Input shape should be (batch_size, channels, height, width)"
        if not eval:
            self.mu = np.mean(x, axis=(0, 2, 3), keepdims=True)
            self.std = np.std(x, axis=(0, 2, 3), keepdims=True)
            self.mu_hat = (1 - self.beta) * self.mu_hat + self.beta * self.mu
            self.std_hat = (1 - self.beta) * self.std_hat + self.beta * self.std
        else:
            self.mu = self.mu_hat
    ...
    def backward(self, **delta):
        ...
        dx_hat = dz * self.vars["W"]
        m = dz.shape[0]
        self.vars["dW"] = np.sum(self.x_hat * dz, axis=(0, 2, 3), keepdims=True) / m
        self.vars["db"] = np.sum(dz, axis=(0, 2, 3), keepdims=True) / m

        delta = (m * dx_hat - np.sum(dx_hat, axis=(0, 2, 3), keepdims=True) - self.x_hat * np.sum(
            dx_hat * self.x_hat, axis=(0, 2, 3), keepdims=True)) / (m * np.sqrt(self.std ** 2 + self.eps))
        return dict(delta=delta)
...
class Flatten(Layer):
    """
    A layer to flatten the input tensor into a 1D vector.
    """
    ...
    def forward(self, x):
        ...
        Returns:
        - flattened: Flattened tensor of shape (batch_size, num_features)
        """
    ...
    def backward(self, **delta):
        ...
        Parameters:
        - delta: Gradient of the loss with respect to the output of the Flatten layer.

        Returns:
        - delta_flattened: Gradient of the loss with respect to the input of the Flatten layer.
        """
        delta = delta["delta"]
        delta_flattened = delta.reshape(delta.shape[0], *self.input_shape[1:])
        return dict(delta=delta_flattened)

    def summary(self):
        """
        Summary of the Flatten layer.

        Returns:
        - name: Name of the layer.
        - output_shape: Shape of the output tensor after flattening.
        - n_param: Number of parameters in the layer (which is usually 0 for a Flatten layer).
        """
        name = self.__class__.__name__
        output_shape = (-1, np.prod(self.input_shape[1:]))
        n_param = 0
        return name, output_shape, n_param
    
    def __call__(self, x):
        """
        Call method to forward input through the Flatten layer.
...
class LSTMCell(ParamLayer, ABC):
    ...
    def forward(self, x, ht_1, ct_1, eval=False):
        ...
        # input = np.hstack([x, ht_1.reshape((batch_size, 1, 100))])
